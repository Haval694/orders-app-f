    ...
    <ScreenContainer edges={["top", "left", "right"]}>
      <View style={styles.page}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => <OrderCard order={item} colors={colors} onEdit={() => openEdit(item)} onDelete={() => confirmDelete(item)} onStatus={(status) => updateOrder(item.id, { ...item, status, finalAmount: calculateFinalAmount(item.productPrice, item.deliveryFee) })} />}
